...
def run(
    ...
    pair_generator = PairGeneratorDataset.load(json.load(Path(f"./data/{data}/train.json").open("r")))

    for layer_nb in layer_nbs:
        module_name = f"transformer.h.{layer_nb}"
        layer = model.get_submodule(module_name)
        
        dirs = DirFinder(model, layer, pair_generator, h_size, n_dirs, projection_fn=projection_fn).find_dirs()

        file_name = f"l{layer_nb}-n{n_dirs}-d{data}.pt"
        dir_path = (
            Path(".") / "saved_dirs" / f"v2-{model_name}{cone_suffix}"
        )
